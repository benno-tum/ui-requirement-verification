  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        {selectedFlow && viewMode === 'verification' && <VerificationPanel run={run} onJumpToStep={jumpToStep} />}
...
function VerificationPanel({ run, onJumpToStep }: { run: VerificationRun | null; onJumpToStep: (stepIndex: number) => void }) {
  return (
    <section className="card panel-wide">
      <div className="panel-header">
                ...
                <ul className="evidence-list">
                  {verdict.evidence.map((evidence, index) => (
                    <li key={`${verdict.requirement_id}-${index}`}>
                      <button className="link-button" onClick={() => onJumpToStep(evidence.step_index)}>
    ...
    <article className="requirement-card">
      <div className="requirement-header">
        ...
        <div className="pill-row">
          <span className={`status-pill ${requirement.review_status ?? 'candidate'}`}>{humanizeStatus(requirement.review_status ?? 'candidate')}</span>
          {requirement.manual_verification_label && (
            <span className={`status-pill manual ${requirement.manual_verification_label}`}>manual: {humanizeStatus(requirement.manual_verification_label)}</span>
          )}
          {verdict && <span className={`status-pill ${verdict.label}`}>run: {humanizeStatus(verdict.label)}</span>}
        </div>
      </div>
      <p>{requirement.text}</p>
      <RequirementMeta requirement={requirement} onJumpToStep={onJumpToStep} />
      {requirement.manual_verification_notes && <p className="inline-note">Manual verdict notes: {requirement.manual_verification_notes}</p>}
      {actions}
    </article>
  )
}

function RequirementMeta({ requirement, onJumpToStep }: { requirement: Requirement; onJumpToStep: (stepIndex: number) => void }) {
  return (
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal-card" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="editor-grid">
          <label>
            Requirement text
            <textarea value={form.text} onChange={(event) => setForm({ ...form, text: event.target.value })} rows={4} />
          </label>
